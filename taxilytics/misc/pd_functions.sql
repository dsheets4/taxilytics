...
-- 146: pd_df_to_record
-- ############################################################################
-- DROP FUNCTION IF EXISTS pd_df_to_record(df_bytes bytea, cols text[]);
...
-- DROP FUNCTION IF EXISTS pd_df_slice(df_bytes bytea, start int, end int);
CREATE OR REPLACE FUNCTION pd_slice(df_bytes bytea, start_index integer, end_index integer DEFAULT NULL)
        RETURNS bytea
AS $$
        import pandas as pd
        import pickle
        df = pickle.loads(df_bytes)
        return pickle.dumps(df[start_index:end_index])
$$ LANGUAGE plpython3u;

-- DROP FUNCTION IF EXISTS pd_slice(bytea, integer, integer, text);
CREATE OR REPLACE FUNCTION pd_slice(df_bytes bytea, start_index integer, end_index integer DEFAULT NULL, step_index text DEFAULT NULL)
        RETURNS bytea
AS $$
        import pandas as pd
        import pickle
        ...
        if l < 2:
            ...
            start = start_index
        df = df[start:end:step]
        df.insert(0, 'id', range(0, len(df)))
        return pickle.dumps(df)
$$ LANGUAGE plpython3u;

-- DROP FUNCTION IF EXISTS pd_len(bytea);
CREATE OR REPLACE FUNCTION pd_len(df_bytes bytea)
        RETURNS integer
AS $$
        import pandas as pd
        import pickle
        return len(pickle.loads(df_bytes))
$$ LANGUAGE plpython3u;
...
-- DROP FUNCTION IF EXISTS pd_columns_each(df_bytes bytea);
...
-- -- Using pd_columns_each to get type and column name.
-- WITH one_data AS (
-- 	SELECT * FROM entity_tripdata LIMIT 1
-- )
-- SELECT cols.* FROM one_data, pd_columns_each(_dataframe) as cols;
-- 
-- WITH one_data AS (
-- 	SELECT * FROM entity_tripdata LIMIT 50
-- )
-- SELECT id, array_agg(cols.col_name)
-- FROM one_data, pd_columns_each(_dataframe) as cols
-- GROUP BY id;
-- 
-- WITH one_data AS (
-- 	SELECT * FROM entity_tripdata LIMIT 50
...
-- FROM
-- 	entity_tripdata,
-- 	pd_columns_each(_dataframe) as col_name,
...
-- ############################################################################
-- 137: pd_join - Joins the given data frames
-- ############################################################################
...
-- DROP FUNCTION IF EXISTS pd_join_record(df_bytes_array bytea[], cols text[] DEFAULT ARRAY[]::text[])
CREATE OR REPLACE FUNCTION pd_join_record(df_bytes_array bytea[], cols text[] DEFAULT ARRAY[]::text[])
        RETURNS SETOF RECORD
AS $$
        import pandas as pd
        import pickle
